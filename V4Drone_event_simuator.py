...
class Planner:
    ...
    def greedy_region_assignment(self, K):
        ...
        for _ in range(K):
            ...
            # now flip coin and remove detected events
            for e in list(remaining):
                if e in rps[best_region] and random.random() < rps[best_region][e]:
                    remaining.remove(e)
        return chosen
...
if __name__ == "__main__":
    #  generate a 10×10 grid with base at (0,0)
    ...
    # v1 multi‐run averages (with tqdm)
    stats = {k:[] for k in ('time_left','attempts','detections','combined_prob','runtime_s')}
    for _ in tqdm(range(runs), desc="Running missions"):
    ...
    # v2/v3 region assignment summary at K
    planner = Planner(grid, events)
    ...
    # ——— New: Monte‐Carlo curves at K = [1,4,7,10] —————————————————————
    K_vals   = [1,4,7,10]
    TRIALS   = 500
    o_curve  = [0.0]*len(K_vals)
    g_curve  = [0.0]*len(K_vals)
    r_curve  = [0.0]*len(K_vals)
    for idx, Ki in enumerate(tqdm(K_vals, desc="Curve K's")):
        for _ in range(TRIALS):
            oS, oV = planner.optimal_region_assignment(Ki)
            gS      = planner.greedy_region_assignment(Ki)
            gV      = sum(1 - math.prod([1-x for x in [rps[r].get(e,0) for r in gS]]) for e in events)
            # random baseline
            samp    = random.sample(list(rps), Ki)
            rV      = sum(1 - math.prod([1-x for x in (rps[r].get(e,0) for r in samp)]) for e in events)
    ...
    # normalize
    o_curve = [v/TRIALS for v in o_curve]
    g_curve = [v/TRIALS for v in g_curve]
    r_curve = [v/TRIALS for v in r_curve]

    # Plot 1: all three curves at once
    plt.figure()
    plt.plot(K_vals, g_curve, marker='s')
    plt.title('Greedy Region Assignment')
    plt.xlabel('K')
    ...
    # 2) Optimal only
    plt.figure()
    plt.plot(K_vals, o_curve, marker='o')
    plt.title('Optimal Region Assignment')
    plt.xlabel('K')
    plt.ylabel('Expected detections')
    plt.grid(True)

    # 3) Random baseline only
    plt.figure()
    plt.plot(K_vals, r_curve, marker='^')
    plt.title('Random Baseline Assignment')
    plt.xlabel('K')
    plt.ylabel('Expected detections')
    plt.grid(True)
    ...
    plt.figure()
    plt.plot(K_vals, g_curve, marker='s', label='Greedy')
    plt.plot(K_vals, o_curve, marker='o', label='Optimal')
    plt.plot(K_vals, r_curve, marker='^', label='Random')
    plt.title('Greedy vs Optimal ')
    plt.xlabel('K')
    plt.ylabel('Expected detections')
    plt.legend()
    plt.grid(True)
